...
const ChannelComponent = (props) => {
  const { channel, setChat, userInfo } = props;
  // const [searchResult, setSearchResult] = useState()
  const otherUser =
    channel.channelUsers.find(
  ...
  const lastMessage =
    channel.messages && channel.messages.length
      ...
      : {}
  return (
    <ContactItem onClick={() => setChat({ channelData: channel })}>
...
function ContactListComponent(props) {
  ...
  const refreshContacts = async () => {
    ...
    setChannelList(contactListData.data.responseData)
    setSearchString("")
    setSearchResult("")
  };
  ...
  return (
    <Container>
      ...
      <SearchBox>
        <SearchContainer>
          <SearchIcon src={"https://static.vecteezy.com/system/resources/previews/001/591/517/non_2x/free-search-icon-free-vector.jpg"} />
          <SearchInput
      ...
      {searchResult && (
        <SearchResults>
         <ChannelComponent userData={searchResult} setChat={props.setChat} userInfo={userInfo} />
        </SearchResults>
      )}
